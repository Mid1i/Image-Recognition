...
int main() {
	...
	while (cap.read(frame)) {
		...
		for (const auto& face : faces) {
			...
			for (const auto& eye : eyes) cv::rectangle(faceROI_color, eye, cv::Scalar(0, 0, 255), 2);

			std::vector<cv::Rect> smiles;
			smile_cascade.detectMultiScale(faceROI_gray, smiles, 1.24, 15, 0, cv::Size(40, 40));

			for (const auto& smile : smiles) cv::rectangle(faceROI_color, smile, cv::Scalar(0, 255, 0), 2);
		}
